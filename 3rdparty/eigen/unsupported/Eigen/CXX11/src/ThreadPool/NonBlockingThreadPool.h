...
class ThreadPoolTempl : public Eigen::ThreadPoolInterface {
 public:
  ...
  ThreadPoolTempl(int num_threads, bool allow_spinning,
    ...
    }//all_coprimes_为二维vector，存储与每个i互质的数
#ifndef EIGEN_THREAD_LOCAL
    init_barrier_.reset(new Barrier(num_threads_));
#endif
    thread_data_.resize(num_threads_);//thread_data_为存储结构体ThreadData(230 line)的vector，resize
    for (int i = 0; i < num_threads_; i++) {
      SetStealPartition(i, EncodePartition(0, num_threads_));//SetStealPartition定义在line 193，EncodePartition定义在177,为每个stealpartition设置任务编码
      thread_data_[i].thread.reset(
          env_.CreateThread([this, i]() { WorkerLoop(i); }));//reset将指针指向括号内容
    }
#ifndef EIGEN_THREAD_LOCAL
  ...
  ~ThreadPoolTempl() {
    ...
    if (!cancelled_) {
      ec_.Notify(true);//wakes one or all waiting threads.
    } else {
  ...
  void SetStealPartitions(const std::vector<std::pair<unsigned, unsigned>>& partitions) {//为每个thread设置可窃取的分区
    eigen_plain_assert(partitions.size() == static_cast<std::size_t>(num_threads_));//static_cast类型转换 int->size_t
    ...
    for (int i = 0; i < num_threads_; i++) {
      ...
      SetStealPartition(i, val);//将每个可窃取的Partition的编码存入到相关内存中
    }
  }
 ...
 private:
  ...
  static const int kMaxPartitionBits = 16;//让线程数小于unsigned所表示最大
  static const int kMaxThreads = 1 << kMaxPartitionBits;

  inline unsigned EncodePartition(unsigned start, unsigned limit) {
    return (start << kMaxPartitionBits) | limit;
  }//对start和limit进行编码，前16位表示start，后16位表示limit
  ...
  inline void SetStealPartition(size_t i, unsigned val) {
    thread_data_[i].steal_partition.store(val, std::memory_order_relaxed);//store是将val按原子顺畅存储到共享内存中
  }

  inline unsigned GetStealPartition(int i) {
    return thread_data_[i].steal_partition.load(std::memory_order_relaxed);//load是将内存中的值读取出来。
  }
//memory_order_relaxed: 只保证当前操作的原子性，不考虑线程间的同步，其他线程可能读到新值，也可能读到旧值。
  ...
  // Main worker thread loop.主要是初始化相应的线程以及每一个线程的任务、并且定义窃取任务的逻辑
  void WorkerLoop(int thread_id) {
#ifndef EIGEN_THREAD_LOCAL
    std::unique_ptr<PerThread> new_pt(new PerThread());
    per_thread_map_mutex_.lock();
    bool insertOK = per_thread_map_.emplace(GlobalThreadIdHash(), std::move(new_pt)).second;//emplace当且仅当键未存在时才可插入，GlobalThreadIdHash()定义在第454行，其返回一个线程的唯一标识，std::move是对右值的转移
    ...
    init_barrier_->Wait();
#endif
    PerThread* pt = GetPerThread();//如果未定义EIGEN_THREAD_LOCAL，则在per_thread_map_中查找当前线程，若没有，则返回dummy，否则返回该线程；若定义了该线程，则返回EIGEN_THREAD_LOCAL的指针
    pt->pool = this;
  ...
  // Steals work within threads belonging to the partition.（从大小核分区中偷）
  Task LocalSteal() {
    PerThread* pt = GetPerThread();
  ...
  static EIGEN_STRONG_INLINE uint64_t GlobalThreadIdHash() {
    return std::hash<std::thread::id>()(std::this_thread::get_id());//std::thread::id专门初始化hash，用std::this_thread::get_id()生成thread中的唯一标识_M_thread，https://blog.csdn.net/FlushHip/article/details/81902188
  }
  ...
  EIGEN_STRONG_INLINE PerThread* GetPerThread() {
    ...
    return pt;
#endif
  }//如果未定义EIGEN_THREAD_LOCAL，则在per_thread_map_中查找当前线程，若没有，则返回dummy，否则返回该线程；若定义了该线程，则返回EIGEN_THREAD_LOCAL的指针
